...
- (void) configureView
{
    ...
    _lowerLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, - 20, 40, 20)];
    _lowerLabel.text = @"0";
    _lowerLabel.backgroundColor = [UIColor colorWithRed:59/255.f green:201/255.f blue:114/255.f alpha:1];
    ...
    _upperLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, - 20, 40, 20)];
    _upperLabel.text = @"0";
    _upperLabel.backgroundColor = [UIColor colorWithRed:59/255.f green:201/255.f blue:114/255.f alpha:1];
...
- (void) observeValueForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void*)context {
    if ([keyPath isEqual:@"frame"]) {
        if (object == self.lowerHandle) {
            ...
            _lowerLabel.center = self.lowerCenter;
            _lowerLabel.transform = CGAffineTransformMakeTranslation(0, -20);
        } else if (object == self.upperHandle) {
            self.upperCenter = self.upperHandle.center;
            _upperLabel.center = self.upperCenter;
            _upperLabel.transform = CGAffineTransformMakeTranslation(0, -20);
        }
...
- (void) setLowerValue:(float)lowerValue
{
    ...
    _lowerLabel.text = [NSString stringWithFormat:@"%ld",[self getLabelText:value]];
...
- (void) setUpperValue:(float)upperValue
{
    ...
    _upperLabel.text = [NSString stringWithFormat:@"%ld",[self getLabelText:value]];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    [self setLowerValue:[self getNearestNum:_lowerValue] animated:YES];
    [self setUpperValue:[self getNearestNum:_upperValue] animated:YES];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    self.lowerLabel.alpha = 1.f;
    self.upperLabel.alpha = 1.f;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (tickNumbers) {
        ...
        NSString *firstItem = tickNumbers.firstObject;
        NSString *lastItem = tickNumbers.lastObject;
        
        space = (self.frame.size.width)/(lastItem.integerValue - firstItem.integerValue);
        CGFloat lineSpace = (self.frame.size.width - 5)/(lastItem.integerValue - firstItem.integerValue);

        minValue = space/(self.frame.size.width);
        ...
        for (NSString *temp in tickNumbers) {
            ...
            [temp drawInRect:CGRectMake(0 + space*(current - firstItem.integerValue), 0, 20, 20) withAttributes:@{NSFontAttributeName:font}];
            
            //开始一个起始路径
            CGContextBeginPath(context);
            //起始点设置为(0,0):注意这是上下文对应区域中的相对坐标，
            CGContextMoveToPoint(context, 12 + (current - firstItem.integerValue)*space, 20);
            //设置下一个坐标点
            CGContextAddLineToPoint(context, 12 + space*(current - firstItem.integerValue), 30);
            //连接上面定义的坐标点
...
- (CGFloat )getNearestNum:(CGFloat )value{
    NSInteger location = ((value)/minValue) + [tickNumbers.firstObject integerValue];//获取thumb所在的
    rangeModel = [SliderRange new];
    
    if (tickNumbers && tickNumbers.count > 0) {
        for (NSString *temp in tickNumbers) {
            NSInteger index = [tickNumbers indexOfObject:temp];

            if (temp.integerValue > location ) {
                rangeModel.upperNum = temp.integerValue;
                
                if (index != 0) {
                    rangeModel.lowerNum = [tickNumbers[index - 1] integerValue];
                }else{
                    rangeModel.lowerNum = 0;
                    rangeModel.upperNum = 0;
                }
                
                if (location - rangeModel.lowerNum > rangeModel.upperNum - location) {
                    return MIN(1, rangeModel.upperNum*minValue);
                }else{
                    return rangeModel.lowerNum*minValue;
                }
            }
        }
...
- (NSInteger)getLabelText:(CGFloat)value{
    return round((value)/minValue);
}
@end

@implementation SliderRange
